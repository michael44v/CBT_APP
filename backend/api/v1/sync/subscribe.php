...
<script>
    ...
    try {
        console.log("Loading Paystack configuration...");

        const res = await fetch("http://localhost:80/fillop/api/v1/payments/config.php", {
            cache: "no-store"
        });
        ...
        if (!data.success || !data.public_key) {
            throw new Error("Paystack public key was not returned.");
        }
    ...
    document.getElementById("subscribeForm").addEventListener("submit", async (e) => {
        ...
        try {
            const res = await fetch("http://localhost:80/fillop/api/v1/register.php", {
                method: "POST",
            ...
            if (data.success && data.reference) {
                ...
                if (data.amount > 0) {
                    ...
                    if (typeof PaystackPop === 'undefined') {
                        alert("Paystack SDK has not loaded.");
    ...
    async function verifyAndFinishRegistration(ref) {
        ...
        try {
            const vRes = await fetch("http://localhost:80/fillop/api/v1/payments/verify.php", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
...
(async function init() {
    ...
    const paystackReady = await loadPaystackConfig();

    await loadPricingSettings();

    console.log("Paystack SDK:", typeof PaystackPop);
    console.log("Paystack key:", paystackPublicKey);
